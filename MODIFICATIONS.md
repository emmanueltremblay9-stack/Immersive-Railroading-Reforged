...
| `build.gradle` | Preserve UMC-generated NeoForge scaffold, bump the test version for each verified install pass, and align Java toolchain/release target with Java 21. |
...
| `install-mod.ps1` | Add repeatable Windows build/install verification for the Prism LAB mods folder, including runtime JAR selection, old same-mod JAR removal, and required dependency copying into `build/libs` with hash checks and `build/install-report.json`. |
