...
| `install-mod.ps1` | Add repeatable Windows build/install verification for the Prism LAB mods folder, including runtime JAR selection, old same-mod JAR removal, required dependency copying, fixed project `libs` mirroring, hash checks, and `build/install-report.json`. |
| `libs/README.md` | Document the local ignored JAR mirror populated by `install-mod.ps1`. |
